...
function createBoard() {
	...
	gameBoard.innerHTML = "";
	initBoard.forEach((startPiece, i) => {
...
function handleSwapClick(event) {
	...
	if (selectedSquareId === targetSquareId)
		return;
...
function swapPieces(fromSquareId, toSquareId) {
	...
	const fromPiece = fromSquare.innerHTML;
	const toPiece = toSquare.innerHTML;

	fromSquare.innerHTML = toPiece;
	toSquare.innerHTML = fromPiece;
...
function deflectBullet(piece, position, bulletDiv, player) {
	...
	if (newBulletDirection === null) {
		destroyPiece(position, bulletDiv);
		bulletMoving = false;
		return;
	}
...
function destroyPiece(squareId, bulletDiv) {
...
function saveMoveHistory(fromSquareId, toSquareId, rotation = 0, moveType = "move") {
	const move = {
		from: fromSquareId,
		to: toSquareId,
		rotation,
...
function updateTimerDisplay() {
	...
	const blueMinutes = Math.floor(timers.blue / 60).toString().padStart(2, '0');
	const blueSeconds = (timers.blue % 60).toString().padStart(2, '0');
	const redMinutes = Math.floor(timers.red / 60).toString().padStart(2, '0');
